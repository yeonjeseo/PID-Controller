...
#define SBUS_min        352
#define maxStickAng     45         // Max Angle[Degree]
...
float xPIDgain[3]={0.5, 0, 0.25};    // X axis P-I-D
float yPIDgain[3]={0, 0, 0};        // Y axis P-I-D
float zPIDgain[3]={0, 0, 0};       // Z axis P-I-D
...
void Control(void)
{
    ...
    //for(i=0;i<4;i++) Motor[i] = tmp[2]*SBUS_gain + Motor_N;     
    ...
    if(sbus[4]<1000)
    {
        ...
        Thrust[0] = REF_Thrust + Roll_ctrl;
        Thrust[1] = REF_Thrust - Roll_ctrl;
        Thrust[2] = REF_Thrust - Roll_ctrl;
        Thrust[3] = REF_Thrust + Roll_ctrl;
...
float PID_Control(int N, float Command, float state, float derivative, float* Gain)
{
    ...
    Dctrl[N] = Gain[2]*(-derivative/(72.8));
...
interrupt [TIM1_COMPA] void timer1_init(void)                   // 1ms Transmit (SPI 1Byte씩 송신)
{   
    if(tx_flag==1)
    {               
